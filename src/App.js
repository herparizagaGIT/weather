...
const Home = () => {
    ...
    return (
        <main>
            <h1 id="MAIN_TITLE">Conoce el clima de {cities.length} ciudades del mundo</h1>
            {cities.map((ciudad, i) => <div key={i} className="BOX"><Link to={"/ciudad/" + ciudad.city} className="selected"><h1>{ciudad.city}</h1><h2>{ciudad.country}</h2></Link></div>)}
        </main>
    );
};
